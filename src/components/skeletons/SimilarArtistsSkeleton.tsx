import { Skeleton } from "@/components/ui/skeleton";

export const SimilarArtistsSkeleton = () => {
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-8 mb-16 items-stretch">
      {[...Array(4)].map((_, i) => (
        <div 
          key={i}
          className="relative glass-modern rounded-2xl overflow-hidden border border-white/10 h-full min-h-[480px] flex flex-col"
        >
          {/* Image Skeleton */}
          <div className="h-48 w-full relative">
            <Skeleton className="h-full w-full bg-muted/20" />
            {/* Badges */}
            <Skeleton className="absolute top-4 right-4 h-8 w-12 rounded-lg bg-muted/30" />
            <Skeleton className="absolute top-4 left-4 h-8 w-20 rounded-lg bg-muted/30" />
          </div>
          
          {/* Content Skeleton */}
          <div className="p-6 flex-1 flex flex-col bg-background/95">
            {/* Name */}
            <Skeleton className="h-8 w-3/4 mb-4 bg-muted/30" />
            
            {/* Location */}
            <div className="flex items-center mb-6">
              <Skeleton className="w-4 h-4 mr-2 rounded-full bg-muted/30" />
              <Skeleton className="h-4 w-1/2 bg-muted/30" />
            </div>
            
            {/* Specialties */}
            <div className="flex flex-wrap gap-2 mb-8">
              <Skeleton className="h-6 w-20 rounded-md bg-muted/20" />
              <Skeleton className="h-6 w-24 rounded-md bg-muted/20" />
              <Skeleton className="h-6 w-16 rounded-md bg-muted/20" />
            </div>

            {/* Price */}
            <div className="mt-auto mb-6">
              <div className="flex items-baseline gap-2 pb-4 border-b border-white/5">
                <Skeleton className="h-8 w-32 bg-muted/30" />
              </div>
            </div>
            
            {/* Button */}
            <Skeleton className="h-12 w-full rounded-lg bg-muted/20" />
          </div>
        </div>
      ))}
    </div>
  );
};
